...
def extrair_informacoes(link, linha):
    ...
    nome_produto = site.find(class_='my-2')
    preco_produto = site.find(class_='price-item price-item--regular font-extrabold text-[#3D3D3D] text-[22px]')
    sku_produto = site.find(class_='text-[32px] leading-[42px] font-black text-[#3D3D3D] mb-0')
    image_tag = site.find('img', class_='ls-is-cached lazyloaded') 
    ...
    if preco_produto.strip():
        ...
        valores = (
            ...
            'Boss Audio'  # Valor da nova coluna origem
        )
        ...
        sheet[f'F{proxima_linha}'] = 'Boss Audio'
...
def configure_spreadsheets():
    try:
        ...
        planilha = pd.read_excel('03.Input_links_bossaudio.xlsx')

        # Carregando a planilha de produtos existente
        workbook = openpyxl.load_workbook('02.Output_informacoes_produtos_bossaudio.xlsx')
...
if __name__ == "__main__":
    try:
        ...
        # Salvando a planilha
        workbook.save('02.Output_informacoes_produtos_bossaudio.xlsx')

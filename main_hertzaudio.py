...
def extrair_informacoes(link, linha):
    ...
    if preco_produto.strip():
        ...
        sql = """
        INSERT INTO products_price_hertzaudio (nome_produto, preco_produto, sku_produto, image_link, data_extracao, origem)
        VALUES (?, ?, ?, ?, ?, ?)
        """
        valores = (
            ...
            'Hertz Audio'  # Valor da nova coluna origem
        )
        ...
        sheet[f'F{proxima_linha}'] = 'Hertz Audio'  # Valor da nova coluna origem
...
def configure_spreadsheets():
    try:
        ...
        planilha = pd.read_excel('03.Input_links_hertzaudio.xlsx')

        # Carregando a planilha de produtos existente
        workbook = openpyxl.load_workbook('02.Output_informacoes_produtos_hertzaudio.xlsx')
...
if __name__ == "__main__":
    try:
        ...
        # Salvando a planilha
        workbook.save('02.Output_informacoes_produtos_hertzaudio.xlsx')

...
def nova_extracao(link, cursor, sheet):
    try:
        ...
        nome_produto = site.find(class_='h1 mt-0 mb-4 js-product-name text-center')

        preco_elemento = site.find('h4', class_='js-price-display')
        preco_produto_text = preco_elemento.get_text().replace('R$', '') if preco_elemento else None
        preco_produto = float(preco_produto_text.replace('.', '').replace(',', '.')) if preco_produto_text else None

        sku_produto = 'Não tem'

        image_link = site.select_one('.js-product-slide a')['href']

        # Inserção dos dados no banco de dados
        sql = """
        INSERT INTO products_price_bomber (nome_produto, preco_produto, sku_produto, image_link, data_extracao, origem)
        VALUES (?, ?, ?, ?, ?, ?)
        """
        valores = (
        ...
        sheet[f'F{proxima_linha}'] = 'Bomber Brasil'
...
def configure_spreadsheets():
    try:
        ...
        planilha = pd.read_excel('03.Input_links_Bomber.xlsx')

        # Carregando a planilha de produtos existente
        workbook = openpyxl.load_workbook('02.Output_informacoes_produtos_Bomber.xlsx')
...
if __name__ == "__main__":
    try:
        ...
        # Salvando a planilha
        workbook.save('02.Output_informacoes_produtos_Bomber.xlsx')

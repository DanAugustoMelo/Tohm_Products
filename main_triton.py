...
def nova_extracao(link, cursor, sheet):
    try:
        ...
        sql = """
        INSERT INTO products_price_triton (nome_produto, preco_produto, sku_produto, image_link, data_extracao, origem)
        VALUES (?, ?, ?, ?, ?, ?)
        """
        valores = (
            ...
            'Triton ML'  # Valor da nova coluna origem
        )
        ...
        sheet[f'F{proxima_linha}'] = 'Triton ML'  # Valor da nova coluna origem
...
def configure_spreadsheets():
    try:
        ...
        planilha = pd.read_excel('Produtos_Triton.xlsx')

        # Carregando a planilha de produtos existente
        workbook = openpyxl.load_workbook('02.Output_informacoes_produtos_triton.xlsx')
...
if __name__ == "__main__":
    try:
        ...
        # Salvando a planilha
        workbook.save('02.Output_informacoes_produtos_triton.xlsx')

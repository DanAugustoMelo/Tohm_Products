...
def nova_extracao(link, cursor, sheet):
    try:
        ...
        nome_produto = site.find(class_='prod-name')
        preco_produto = site.find('span', id='variacaoPreco')
        sku_produto = site.find('strong', class_='dados-valor')
        image_tag = site.find('img', class_='thumb change-photo slick-slide slick-current slick-active')
        ...
        if preco_produto.strip():
            ...
            preco_final = float(preco_produto)

        # Inserção dos dados no banco de dados
        sql = """
        INSERT INTO products_price_faital (nome_produto, preco_produto, sku_produto, image_link, data_extracao, origem)
        VALUES (?, ?, ?, ?, ?, ?)
        """
        valores = (
            ...
            'Amerco Brasil'  # Valor da nova coluna origem
        )
        ...
        sheet[f'B{proxima_linha}'] = preco_final if preco_final != '' else ''
        ...
        sheet[f'F{proxima_linha}'] = 'Amerco Brasil'
...
def configure_spreadsheets():
    try:
        ...
        planilha = pd.read_excel('03.Input_links_faital.xlsx')

        # Carregando a planilha de produtos existente
        workbook = openpyxl.load_workbook('02.Output_informacoes_produtos_faital.xlsx')
...
if __name__ == "__main__":
    try:
        ...
        # Salvando a planilha
        workbook.save('02.Output_informacoes_produtos_faital.xlsx')

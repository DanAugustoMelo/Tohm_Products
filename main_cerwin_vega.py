...
if __name__ == "__main__":
    ...
    def extrair_informacoes(link, linha):
        ...
        nome_produto = site.find(class_='product-item-link')
        preco_produto = site.find(class_='price')
        sku_produto = site.find(class_='product sku product-item-sku')
        image_tag = site.find('img', class_='product-image-photo')
    ...
    planilha = pd.read_excel('Input_links_cerwin_vega.xlsx')

    # Carregando a planilha de produtos existente
    workbook = openpyxl.load_workbook('Output_informacoes_produtos_cerwin_vega.xlsx')
    ...
    # Salvando a planilha
    workbook.save('Output_informacoes_produtos_cerwin_vega.xlsx')

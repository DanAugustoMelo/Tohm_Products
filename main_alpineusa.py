...
def extrair_informacoes(link, linha):
    ...
    nome_produto = site.find(class_='productHero__copy productHero__copy__desktop')
    preco_produto = site.find(class_='productHero__price productHero__promo_price productHero__copy__desktop')
    sku_produto = site.find(class_='productHero__name productHero__copy__desktop')
    
    # Procurar a tag de imagem com a classe 'productHero__image'
    image_tag = site.find('img', class_='productHero__image') 

    # Verificar se a tag de imagem foi encontrada
    if image_tag:
        image_link = image_tag.get('src', '')  # Usando .get() para evitar erros caso 'src' não esteja presente
    else:
        image_link = ''
    ...
    if preco_produto:
        ...
        preco_texto = re.sub(r'[^\d,\.]', '', preco_texto)  # Remover caracteres indesejados
    ...
    if preco_produto.strip():
        ...
        preco_produto = 0.0  # Definindo o preço como 0.0 caso não haja valor numérico

        # Inserção dos dados no banco de dados (ADICIONAR ORIGEM)
        sql = """
        INSERT INTO products_price_alpineusa (nome_produto, preco_produto, sku_produto, image_link, data_extracao, origem)
        VALUES (?, ?, ?, ?, ?, ?)
        """
        valores = (
            ...
            'Alpine USA'  # Valor da nova coluna origem
        )
        ...
        sheet[f'F{proxima_linha}'] = 'Alpine USA'
...
def configure_spreadsheets():
    try:
        ...
        planilha = pd.read_excel('03.Input_links_alpineusa.xlsx')

        # Carregando a planilha de produtos existente
        workbook = openpyxl.load_workbook('02.Output_informacoes_produtos_alpineusa.xlsx')
...
if __name__ == "__main__":
    try:
        ...
        # Salvando a planilha
        workbook.save('02.Output_informacoes_produtos_alpineusa.xlsx')
